...
const GptSearchBar = () => {
    ...
    const previousMovieNamesRef = useRef(null);

    useEffect(() => {
        // Check if movieNames actually changed and is not empty
        if (!movieNames || 
            !Array.isArray(movieNames) || 
            movieNames.length === 0 || 
            !isVoiceEnabled || 
            hasSpokenRef.current ||
            JSON.stringify(previousMovieNamesRef.current) === JSON.stringify(movieNames)) {
            return;
        }

        // Update the previous movieNames reference
        previousMovieNamesRef.current = movieNames;
        
        // Speak the text
        SpeakText(movieNames);
        
        // Reset the voice enabled state and mark as spoken
    ...
    const handleRemoveMovies = () => {
        ...
        previousMovieNamesRef.current = null; // Reset previous reference
        setIsVoiceEnabled(false); // Reset voice enabled state
        setErrorMessage("");
    }
    ...
    const RecordSpeech = () => {
        ...
        r.onresult = function (event) {
            ...
            // Reset the spoken flag before new search
            hasSpokenRef.current = false;
            
            // Call GPT logic and then enable voice
            handleGptSearchClick().then(() => {
    ...
    const handleGptSearchClick = async () => {
        ...
        try {
            ...
            const geminiPrompt = "Act as an Movie Recommendation System and suggest movies for the query " + 
                searchText.current.value + 
                " only give me names of six top rated Movies and give me movies in comma separated format like the example result given ahead. Example Result : Gadar, Sholay , Don , Golmal, Kuch kuch Hota hai";
            ...
            const gptMovies = gptResponse.text.split(",").map(movie => movie.trim());
